/**
 * Stable-key audit — runs against consecutive Pokégenie CSV exportData.
 * For each N→N+1 pair:
 *   1. Builds stable keys for both exportData (matching analyse.js makeStableKey)
...
console.log(`\nFound ${csvFiles.length} CSV exportData in: ${folder}`);
csvFiles.forEach((f, i) => console.log(`  [${i}] ${f}`));
console.log('');

// Parse all files
const exportData = csvFiles.map(f => {
...
for (let i = 0; i < exportData.length - 1; i++) {
  const A = exportData[i];
  const B = exportData[i + 1];
  ...
  // Pokémon present in both exportData
  ...
  sharedNids.forEach(nid => {
    const as = natA[nid];
    const bs = natB[nid];
    // Pair them up by position (same order in both exportData)
...
if (totalProblems === 0) {
  console.log('\n✅ Stable key is working correctly across all exportData');
} else {
