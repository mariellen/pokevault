...
| `best_overall` | Best-IV per species with no confirmed league slot — **all species** (legendary and non-legendary; non-legendaries must qualify ≥90% in some league and have no confirmed family keeper unless `masterDemoted`). Also suppressed for a genuine **pre-evolution of the family's confirmed Master keeper** (#46) — checked via `VALID_EVOLUTIONS` evolves-into, by ANY keeper mechanism (ordinary pool winner, Dmax, Gmax), not a flag whitelist; does NOT suppress a `masterDemoted` sibling final evolution (branching families, e.g. Glaceon losing Master to Vaporeon — neither is the other's ancestor, both still surface). Nick: `NameⓇ{IV%}` |
